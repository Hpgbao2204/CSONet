...
\end{minipage}
\caption{Component ablation with a shared configuration order and plotting grammar.  Panel (a) connects precision, recall, and \(F_1\), with shading spanning their local range. Panel (b) contrasts accuracy, coverage, and accuracy conditional on definite verdicts; its band exposes the accuracy--coverage decision gap. Panel (c) combines median and p95 latency, an IQR band, and the p95/median tail-inflation ratio on the right axis; compilation and Anvil replay are excluded. Classification curves use Jeffreys estimates, whereas timing layers use raw empirical quantiles.}
\label{fig:diagnostics}
\end{figure*}

The full configuration has raw precision 0.936, recall 0.850, \(F_1=0.891\), accuracy 0.700, and decision coverage 0.739.  Removing behavioral reasoning lowers recall to 0.450, while identifier-only layout matching reaches 0.483; Fig.~\ref{fig:diagnostics}(a) shows that the loss is primarily sensitivity rather than precision.  Slot--offset--type matching retains strong precision but reaches recall 0.750 because it cannot recover assembly evidence.  Figure~\ref{fig:diagnostics}(b) separates overall accuracy from correctness conditional on a definite verdict: analyzing all functions preserves the main detection metrics but lowers coverage to 0.606 by moving otherwise skippable inputs to \(\Unknown\).
