...
\centering
\includegraphics[width=\linewidth]{figure/figure_storage_1a_profile.pdf}\\[-1mm]
\scriptsize (a) Storage detection
\end{minipage}\hfill
\begin{minipage}[t]{0.238\textwidth}
\centering
\includegraphics[width=\linewidth]{figure/figure_storage_1b_cumulative.pdf}\\[-1mm]
\scriptsize (b) Ablation profile
\end{minipage}\hfill
\begin{minipage}[t]{0.238\textwidth}
\centering
\includegraphics[width=\linewidth]{figure/figure_storage_1c_sensitivity.pdf}\\[-1mm]
\scriptsize (c) Layout latency
\end{minipage}\hfill
\begin{minipage}[t]{0.238\textwidth}
\centering
\includegraphics[width=\linewidth]{figure/figure_storage_1d_operators.pdf}\\[-1mm]
...
\begin{minipage}[t]{0.238\textwidth}
\centering
\includegraphics[width=\linewidth]{figure/figure_behavior_2a_profile.pdf}\\[-1mm]
\scriptsize (e) Outcome profile
\end{minipage}\hfill
\begin{minipage}[t]{0.238\textwidth}
\centering
\includegraphics[width=\linewidth]{figure/figure_behavior_2b_latency.pdf}\\[-1mm]
\scriptsize (f) Verification density
\end{minipage}\hfill
\begin{minipage}[t]{0.238\textwidth}
\centering
\includegraphics[width=\linewidth]{figure/figure_behavior_2c_replay.pdf}\\[-1mm]
\scriptsize (g) Replay quantiles
\end{minipage}\hfill
\begin{minipage}[t]{0.238\textwidth}
\centering
\includegraphics[width=\linewidth]{figure/figure_behavior_2d_observables.pdf}\\[-1mm]
...
The hardened analyzer detected 48/60 behavioral regressions and returned \(\Unknown\) for the remaining 12 cases, with no incorrect-safe outcome or timeout.  Raw recall remains 0.800 and exceeds the deterministic differential baseline's 0.733.  Arithmetic mutations account for the four additional detections in Fig.~\ref{fig:effectiveness}(e), while the three hash-based returns remain explicitly unsupported.
...
\centering
\includegraphics[width=\linewidth]{figure/figure_diagnostics_3a_quality.pdf}\\[-1mm]
\scriptsize (a) Precision, recall, and \(F_1\)
\end{minipage}\hfill
\begin{minipage}[t]{0.322\textwidth}
\centering
\includegraphics[width=\linewidth]{figure/figure_diagnostics_3b_decision.pdf}\\[-1mm]
\scriptsize (b) Accuracy and coverage
\end{minipage}\hfill
\begin{minipage}[t]{0.322\textwidth}
\centering
\includegraphics[width=\linewidth]{figure/figure_diagnostics_3c_runtime.pdf}\\[-1mm]
\scriptsize (c) Median and tail latency
\end{minipage}
\caption{Component ablation with a shared configuration order and plotting grammar.  Panels (a)--(b) report the primary classification metrics and decided-case accuracy.  Panel (c) contrasts median with p95 analyzer latency; compilation and Anvil replay are not included.  Curves connect Jeffreys estimates or measured timing summaries; raw classification metrics remain in the text and CSV.}
...
\centering
\includegraphics[width=\linewidth]{figure/figure_scalability_obligations.pdf}\\[-1mm]
\scriptsize (a) Relational obligations
\end{minipage}\hfill
\begin{minipage}[t]{0.485\textwidth}
\centering
\includegraphics[width=\linewidth]{figure/figure_scalability_paths.pdf}\\[-1mm]
...
An earlier audited revision produced nine incorrect-safe outcomes because conditional state updates, conditional events, and changed revert payloads disappeared during summary canonicalization.  The hardened artifact now blocks those cases as \(\Unknown\), and its regression suite checks that none can again become \(\Safe\).  A future control-flow-sensitive encoding is required before these cases can receive definite verdicts.
...
\textbf{Construct validity.} Mutation labels represent controlled hazards, not incident frequency. The deterministic fuzzer is a bounded baseline and is not Forge's built-in fuzzer.  Every \(\Safe\) verdict means ``Safe within the encoded model,'' not unconditional Solidity safety.
