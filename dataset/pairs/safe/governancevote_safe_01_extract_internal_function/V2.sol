...
contract GovernanceVote is EpochState, GuardianState {
    ...
    function addVotingPower(uint256 amount) external returns (uint256) {
        require(active, "inactive");
        require(amount <= uint256(limit), "over limit");
        uint256 next = total + amount;
        _storeTotal(next);
        powers[msg.sender] += amount;
    ...
    function withdraw(address account, uint256 amount) external onlyOwner returns (uint256) {
        require(powers[account] >= amount, "insufficient");
        powers[account] -= amount;
        total -= amount;
