...
contract VaultGuard is EpochState, GuardianState {
    ...
    function depositValue(uint256 amount) external returns (uint256) {
    ...
    function withdraw(address account, uint256 amount) external onlyOwner returns (uint256) {
        require(shares[account] >= amount, "insufficient");
