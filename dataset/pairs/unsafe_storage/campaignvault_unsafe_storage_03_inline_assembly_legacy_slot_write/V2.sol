...
contract CampaignVault is EpochState, GuardianState {
    ...
    function backCampaign(uint256 amount) external returns (uint256) {
        require(active, "inactive");
        require(amount <= uint256(limit), "over limit");
        uint256 next = total + amount;
        total = next;
        backings[msg.sender] += amount;
    ...
    function withdraw(address account, uint256 amount) external onlyOwner returns (uint256) {
        require(backings[account] >= amount, "insufficient");
        backings[account] -= amount;
        total -= amount;
