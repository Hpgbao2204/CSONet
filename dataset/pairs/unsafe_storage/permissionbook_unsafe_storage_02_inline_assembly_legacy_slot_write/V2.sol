...
contract PermissionBook is EpochState, GuardianState {
    ...
    function addAllowance(uint256 amount) external returns (uint256) {
    ...
    function withdraw(address account, uint256 amount) external onlyOwner returns (uint256) {
        require(allowances[account] >= amount, "insufficient");
        allowances[account] -= amount;
        total -= amount;
