...
contract IncentiveVault is EpochState, GuardianState {
    ...
    function fundIncentive(uint256 amount) external returns (uint256) {
        require(active, "inactive");
        require(amount <= uint256(limit), "over limit");
        uint256 next = total + amount;
        total = next;
        incentives[msg.sender] += amount;
    ...
    function withdraw(address account, uint256 amount) external onlyOwner returns (uint256) {
        require(incentives[account] >= amount, "insufficient");
        incentives[account] -= amount;
        total -= amount;
    ...
    function overwriteComputedSlot(uint256 value) external onlyOwner {
        assembly {
            let target := sub(3, 1)
            sstore(target, value)
        }
