...
contract DelegatedStake is EpochState, GuardianState {
    ...
    function delegateUnits(uint256 amount) external returns (uint256) {
    ...
    function withdraw(address account, uint256 amount) external onlyOwner returns (uint256) {
        require(delegations[account] >= amount, "insufficient");
        delegations[account] -= amount;
        total -= amount;
