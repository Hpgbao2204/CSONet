...
contract MilestoneEscrow is EpochState, GuardianState {
    ...
    function fundMilestone(uint256 amount) external returns (uint256) {
    ...
    function withdraw(address account, uint256 amount) external onlyOwner returns (uint256) {
        require(milestones[account] >= amount, "insufficient");
        milestones[account] -= amount;
        total -= amount;
