...
contract EscrowDesk is EpochState, GuardianState {
    ...
    function fundEscrow(uint256 amount) external returns (uint256) {
    ...
    function withdraw(address account, uint256 amount) external onlyOwner returns (uint256) {
        require(escrows[account] >= amount, "insufficient");
        escrows[account] -= amount;
        total -= amount;
