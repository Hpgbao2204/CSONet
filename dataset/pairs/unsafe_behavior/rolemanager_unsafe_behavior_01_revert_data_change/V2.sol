...
contract RoleManager is EpochState, GuardianState {
    ...
    function grantQuota(uint256 amount) external returns (uint256) {
    ...
    function withdraw(address account, uint256 amount) external onlyOwner returns (uint256) {
        require(quotas[account] >= amount, "insufficient");
        quotas[account] -= amount;
        total -= amount;
