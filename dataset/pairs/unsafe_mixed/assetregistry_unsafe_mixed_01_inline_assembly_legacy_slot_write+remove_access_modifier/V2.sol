...
contract AssetRegistry is EpochState, GuardianState {
    ...
    function registerUnits(uint256 amount) external returns (uint256) {
    ...
    function withdraw(address account, uint256 amount) external returns (uint256) {
        require(assets[account] >= amount, "insufficient");
