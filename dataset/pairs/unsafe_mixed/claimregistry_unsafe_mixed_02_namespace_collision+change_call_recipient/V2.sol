...
contract ClaimRegistry is EpochState, GuardianState {
    ...
    function recordClaim(uint256 amount) external returns (uint256) {
    ...
    function withdraw(address account, uint256 amount) external onlyOwner returns (uint256) {
        require(claims[account] >= amount, "insufficient");
